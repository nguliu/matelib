...
namespace lfp::detail
{
    ...
    int createTimerfd()
    {
        ...
        if (timerfd < 0) {
			...
			LOG_ERROR << "syscall timerfd_create error: " << strerror_tl(savedError);
        }
        return timerfd;
    }
    ...
    void readTimerfd(int timerfd)
    {
        ...
        LOG_DEBUG << "readTimerfd: readed " << howmany;

        if (n != sizeof howmany) {
            LOG_ERROR << "readTimerfd error, reads " << n << "bytes instead of 8";
        }
    }
    ...
    void resetTimerfd(int timerfd, Timestamp expiration)
    {
        ...
        newValue.it_value = howMuchTimeFromNow(expiration);  //这里只设置了下一次超市间隔，并未设置重复超时间隔，所以是一次性定时器

        int ret = ::timerfd_settime(timerfd, 0, &newValue, nullptr);
        if (ret) {
            int savedError = errno;
			LOG_ERROR << "syscall timerfd_settime error: " << strerror_tl(savedError);
        }
    }
	...
	cancelingTimers_()
{
	...
	LOG_INFO << "TimerQueue: timerfd = " << timerfd_;
}
...
void TimerQueue::cancelTimerInLoop(TimerId timerId)
{
	...
	TimerSet::iterator it = timers_.find(entry);
	if (it != timers_.end()) {
		delete it->second;
...
void TimerQueue::handleRead()
{
	...
	std::vector<Entry> expired = getExpired(now);	//vector作为返回值不会有内存拷贝
...
std::vector<TimerQueue::Entry> TimerQueue::getExpired(Timestamp now)
{
	...
	// 即*end >= sentry，从而一定有end->first > now
