...
EpollPoller::EpollPoller(EventLoop* loop)
  : ownerLoop_(loop),
	epollfd_(::epoll_create1(EPOLL_CLOEXEC)),	//epollfd_不用设置NONBLOCK
	events_(kInitEventListSize)
{
	if (epollfd_ < 0) {
		int savedError = errno;
		LOG_ERROR << "syscall epoll_create1 error: " << strerror_tl(savedError);
		abort();
	}

	LOG_INFO << "EPollPoller: epollfd = " << epollfd_;
}
...
int EpollPoller::poll(int timeoutInMS, ChannelList* activeChannels)
{
	int numEvents = ::epoll_wait(epollfd_,
								 ...
								 timeoutInMS);
	if (numEvents > 0) {
		LOG_DEBUG << numEvents << " events happended in EPollPoller::poll()";
		...
		LOG_DEBUG << "Nothing happended in EPollPoller::poll()";
	}
	else {
		int savedError = errno;
		LOG_ERROR << "syscall epoll_wait error: " << strerror_tl(savedError);
	}
...
void EpollPoller::updateChannel(Channel* channel)
{
	...
	LOG_DEBUG << "EPollPoller::updateChannel: fd=" << channel->fd()
