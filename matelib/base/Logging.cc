...
Logger::LogLevel Logger::logLevel_ = Logger::INFO;	//日志级别默认为INFO
...
Logger::Logger(const char* file, int line, LogLevel level, bool isAsync)
	...
	basename_()
{
	...
	stream_ << detail::LogLevelName[level];
}
...
void Logger::setOutput(OutputFunc func) {
	detail::g_output = func;
	detail::g_flush = nullptr;
}

void Logger::setFlush(FlushFunc func) { detail::g_flush = func; }

const char* lfp::strerror_tl(int savedErrno)
{
