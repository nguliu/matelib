...
{
//	cout << endl << request.methodToString() << " " << request.path() << " " << request.versionToString() << endl;
//
//	const map<string, string>& headers = request.headers();
...
//	cout << endl;
    ...
    if (request.path() == "/")
    {
        ...
        response->addHeader("Server", "lfp's HttpServer");
        string now = Timestamp::now().toFormattedString();
        response->setBody("<html><head><title>This is a time server</title></head>"
	...
	else if (request.path() == "/html")
    {
		...
		int fd = ::open("webserver/http/html.html", O_RDONLY);
		if (fd > 0) {
...
{
    EventLoop loop;
    HttpServer httpServer(&loop, InetAddress(8888), 5);
