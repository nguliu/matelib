...
void EventLoop::loop()
{
	assert(!looping_);
	assertInLoopThread();
	ASYNC_LOG << "EventLoop[" << this << "] start looping";
	...
	looping_ = false;
	ASYNC_LOG << "EventLoop[" << this << "] stop looping";
}
...
void EventLoop::wakeup()
{
	...
	if (n != sizeof one) {
		ASYNC_LOG << "EventLoop::wakeup() error";
	}
}
...
void EventLoop::handleRead()
{
	...
	if (n != sizeof one) {
		ASYNC_LOG << "EventLoop::handleRead() error";
	}
}
...
{
	if (!isInLoopThread()) {
		ASYNC_LOG << "EventLoop::assertInLoopThread-EventLoop[" << this
